...
def process_from_csv(csv_file, client_url, client_user, client_password, output_base_dir="processed_stations"):
    ...
    for idx, row in df.iterrows():
        ...
        # --- Parse New Offline Paths ---
        xml_path = str(row.get('xml_path', '')).strip()
        mseed_path = str(row.get('mseed_path', '')).strip()
        ...
        try:
            # --- Waveform Loading Logic (Offline vs Online) ---
            st = None
            if mseed_path and mseed_path.lower() != 'nan':
                if os.path.exists(mseed_path):
                    print(f"  -> Loading Waveform (MSEED) from LOCAL file: {mseed_path}")
                    try:
                        st = obspy.read(mseed_path)
                    except Exception as e:
                        print(f"  ❌ Failed to parse local MSEED: {e}")
                else:
                    print(f"  ⚠️ Local MSEED file not found at {mseed_path}. Falling back to API...")
            
            # If no offline file was requested, or if the offline fallback failed, fetch from API
            if st is None:
                print("  -> Downloading Waveform (MSEED) from API...")
            ...
            # --- Inventory Loading Logic (Offline vs Online) ---
            inv = None
            if xml_path and xml_path.lower() != 'nan':
                if os.path.exists(xml_path):
                    print(f"  -> Loading Inventory (XML) from LOCAL file: {xml_path}")
                    try:
                        inv = read_inventory(xml_path)
                    except Exception as e:
                        print(f"  ❌ Failed to parse local XML: {e}")
                else:
                    print(f"  ⚠️ Local XML file not found at {xml_path}. Falling back to API...")
            
            # If no offline file was requested, or if the offline fallback failed, fetch from API
            if inv is None:
                print("  -> Downloading Inventory (XML) from API...")
                inv = download_inventory_from_api(network, station)
                
            if inv is None:
                print(f"  ❌ No inventory available for {station}. Skipping.")
                continue
            ...
            for prefix in channel_prefixes:
                ...
                mseed_path_out = os.path.join(station_dir, f"{file_base}.mseed")
                xml_path_out = os.path.join(station_dir, f"{file_base}.xml")
                
                st_prefix.write(mseed_path_out, format="MSEED")
                if inv_prefix:
                    inv_prefix.write(xml_path_out, format="STATIONXML")
...
if __name__ == "__main__":
    ...
    CSV_PATH = "input_stations copy.csv"
